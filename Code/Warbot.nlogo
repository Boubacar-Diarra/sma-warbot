...
Bases-own [ nb-missiles              ;; le nb de missiles transportes
            ...
            create-ok?               ;; est-ce que la base a le droit de créer des robots ?
            messages                 ;; une file de messages
          ]
;
...
to forward-move [ d ]
  without-interruption [
    ...
    if (fd-ok?) [
      ...
      [
        ;; moins de dégâts sur les bases (mieux protégées)
        ifelse ([breed] of a = Bases)
        [
          ask a [ set energy energy - collision-damage / 10 ]
          set energy 0
        ]
...
to ask-for-energy [b a n]
  if (debug) [show "demande d'énergie"]
  ask b [ if (debug) [show messages] set messages insert-item 0 messages (list 0 a n) if debug [show messages]]
  if (debug) [show "fin de demande d'énergie"]
end

to ask-for-missiles [b a n]
  if (debug) [show "demande de missile"]
  ask b [ set messages insert-item 0 messages (list 1 a n)]
end

to recycle [a new-breed]
  set energy energy + 1000 + [energy] of a
  ask a [die]
end
...
;
; possibilite pour la base de recharger un rocket-launcher avec 'n' nouveaux missiles
; cout = 10 pour le creer
; degats = 100 pour un robot / 20 pour une base
;
to give-missiles [ agent n ]
  if (agent != nobody) and (breed = Bases) and ([breed] of agent = RocketLaunchers) [
    ; si on n'atteint pas le max de missiles autorises et qu'on a suffisamment d'energie
    ; et si les 2 agents sont suffisamment proches l'un de l'autre
    if (distance agent <= 2) and ([nb-missiles] of agent + n <= [max-missiles] of agent) and (energy > missile-cost * n) [
      ; le nb de missiles est incremente de 'n'
      ask agent [set nb-missiles nb-missiles + n]
      ; chaque missile coûte 10 points d'energie a fabriquer
      set energy energy - missile-cost * n
    ]
  ]
